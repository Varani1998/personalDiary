...
const createUsers = async ({ username, city, gender, dateofbirth, password }) => {
  let session = null;
  try {
...
const updateUser = async ({ id, username, city, gender, dateofbirth, password }) => {
  try {
    const user = await UserModel.findById(id);

    user.overwrite({ username, city, gender, dateofbirth, password });
    await user.save();
